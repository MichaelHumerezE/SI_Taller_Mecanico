...
@section('content')
    <form action="{{ route('entradas.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <input type="time" name="hora" class="form-control" placeholder="" value="{{ old('hora') ?? date('H:i:s') }}">
        @error('hora')
            <small>*{{ $message }}</small>
            <br><br>
        @enderror
        ...
        <input type="date" name="fecha" class="form-control" placeholder="" value="{{ old('fecha') ?? date('Y-m-d') }}">
        @error('fecha')
            <small>*{{ $message }}</small>
            <br><br>
        @enderror
